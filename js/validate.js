...
validate.methods = (function() {
  ...
  function genericValidate(selector) {
    $(selector).validate({
      ...
      highlight: function(element, errorClass){
        $(element).attr('aria-invalid', true).parents('.form-field').addClass('has-error');
      },
      unhighlight: function(element, errorClass){
        $(element).removeAttr('aria-invalid').removeAttr('aria-label').parents('.form-field').removeClass('has-error');
      }
    });
    translateValidate();
  }
  ...
  function translateValidate() {
    $.extend($.validator.messages, {
      required: "Este campo n&atilde;o pode ficar em branco",
